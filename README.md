...
Depthwise-separable convs → big parameter/FLOP savings.

Nearest-neighbor upsampling → avoids checkerboard artifacts.

Additive skips → crisp details around small objects without ballooning params.

Small bottleneck (latent_dim) → directly boosts the leaderboard’s LD score.

# Shapes
Forward: forward(x: [B,3,256,256]) → ŷ: [B,3,256,256] with Sigmoid at the end.

Latent dimension exposed: model.latent_dim (also available as model.enc.latent_dim).

Deterministic eval: model.eval() works (no dropout; BN/GroupNorm safe).

# Training notes
Loss: MSE ⊕ L1 (weighted), with option to add ROI-weighted MSE/SSIM later.

Optimizer: AdamW, lr 1e-3, wd 1e-4.

Augmentations: light color jitter/noise OK; avoid heavy warps that move the light.

Eval metric (local): full-frame MSE; server also computes ROI-MSE.